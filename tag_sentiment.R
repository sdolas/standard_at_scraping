...
df$tag = 0
df$tag[str_detect(df$body, "\\bSebastian Kurz")] <- 1
df$tag[str_detect(df$title, "\\bSebastian Kurz")] <- 1
df$tag[str_detect(df$subtitle, "\\bSebastian Kurz")] <- 1
...
pltdata <- filtered_df |>
  filter(new_kicker != "sonstiges" & new_kicker != "") |>
